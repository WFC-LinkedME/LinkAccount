...
* 更新联通SDK
  ...
  tips: 从iOS16.4开始，iOS系统开始废弃CTCarrier功能，并不再提供替代方案。SDK获取运营商信息的功能不再可用，SDK内部采用其他方式处理，调用预取号和号码认证方法时会产生多次回调，用户只需要关心成功回调的code即可。建议采用Xcode14.2或以下版本的XCode打包，生成的包可以在iOS16.4正常运营，并且仍拥有CTCarrier的功能。
...
* 修复SDK第一次初始化成功后立即执行预取号操作会发生预取号失败的问题。

当前版本：2.4.8

* SDK内置授权页面隐私协议可增加多个自定义协议，可通过设置`privacyNames` 和 `privacyUrls` 两个数组来实现 。
* 隐私协议文字增加`privacyHeadIndent` 属性，可调整文字与复选框对齐位置。
* 增加 `showQuotes` 属性，可控制运营商默认隐私协议是否显示书名号。

当前版本：2.4.7

* SDK支持Archive bitcode

当前版本：2.4.6

* 修复内置授权页面使用移动号码登录时可能不显示脱敏手机号的问题。
...
#集成指定版本SDK:
pod 'LinkedME_LinkAccount', '2.4.3'
```
...
2.4（SDK 2.4.3及以后版本）如需支持iOS11及以下系统，需要添加依赖库，在项目设置target -> 选项卡Build Phase -> Linked Binary with Libraries添加如下依赖库: `Network.framework` ，并将Status设置为 `Optional`。
